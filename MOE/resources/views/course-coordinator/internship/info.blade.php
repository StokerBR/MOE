...
@section('content')
    ...
    @php
        ...
        $approved = $internshipCourse->approved;
    @endphp
    ...
    <div class="page page-internships page-create-edit">
        ...
        <div class="page-buttons btn-row">
            ...
            <a href="{{ courseCoordUrl('vagas') }}" type="button" class="btn btn-light" title="Voltar">
                <i class="mdi mdi-arrow-left"></i> Voltar
            </a>

            @if ($approved !== false)
                <form action="{{ courseCoordUrl('vagas/'.$internshipCourse->id.'/rejeitar') }}" method="POST">
                    @csrf

                    <button class="btn btn-gradient-danger h-100" type="submit">Rejeitar</button>
                </form>
            @endif

            @if ($approved !== true)
                <form action="{{ courseCoordUrl('vagas/'.$internshipCourse->id.'/aprovar') }}" method="POST">
                    @csrf
                    <button class="btn btn-gradient-success h-100" type="submit">Aprovar</button>
                </form>
            @endif
